...
export default function HeritageCounter() {
  ...
  return (
    <div className="bg-mbs-blue p-8 md:p-12 rounded-2xl shadow-2xl flex flex-col items-center justify-center text-center space-y-6 overflow-hidden relative border-4 border-mbs-blue/20">
      ...
      <div className="flex gap-2 md:gap-4 justify-center items-center">
        {digits.map((digit, index) => (
          <motion.div
            ...
            className="w-16 h-24 md:w-24 md:h-36 bg-white rounded-lg shadow-inner flex items-center justify-center relative border-b-4 border-slate-300"
          >
            {/* Split line decoration */}
            <div className="absolute w-full h-[2px] bg-slate-100 top-1/2 left-0 -translate-y-1/2 z-0"></div>
            
            <span className="text-4xl md:text-7xl font-black text-mbs-blue relative z-10 font-mono">
              {digit}
            </span>
          </motion.div>
        ))}
      ...
      <motion.div
        ...
        className="space-y-1"
      >
        <p className="text-mbs-yellow text-2xl md:text-4xl font-black uppercase italic tracking-tighter drop-shadow-md">
